...
def add_noise(t, stddev=1e-3, name=None):
    with tf.op_scope([t, stddev], name, "add_noise") as name:
        t = tf.convert_to_tensor(t, name="t")
        gn = tf.random_normal(tf.shape(t), stddev=stddev)
        return tf.add(t, gn, name=name)

def zero_slot(t, name=None):
    with tf.op_scope([t], name, "zero_slot") as name:
        t = tf.convert_to_tensor(t, name="t")
        s = tf.shape(t)[1]
        z = tf.zeros(tf.stack([1, s]))
        return tf.concat(axis=0, values=[z, tf.slice(t, [1, 0], [-1, -1])], name=name)

def pos_enc(sentence_size, embedding_size):
...
class MemN2N(object):
    ...
    def __init__(self, batch_size, vocab_size, sentence_size, memory_size, embedding_size,
        ...
        logits = self.calc_output(self._stories, self._queries) # (batch_size, vocab_size)
        self._logits = logits
    ...
    def calc_output(self, stories, queries):
    ...
    def train(self, stories, queries, answers, learning_rate):
